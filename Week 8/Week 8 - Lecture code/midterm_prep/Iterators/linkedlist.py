...
class LinkedList:
    ...
    def append(self, new_value: Any) -> None:
        ...
        if node is None:
            self.front = LinkedNode(new_value)
            return

        while node.link is not None:
            node = node.link

        # all cases after that
    ...
    def insert(self, cursor: LinkedNode, new_value: Any) -> None:
        ...
        elif self.front.value == cursor.value:
            self.prepend(new_value)
        else:
            node = self.front
            while node.link.value != cursor.value:
                node = node.link
            node.link = LinkedNode(new_value, node.link)
    ...
    class _Iter:
        __slots__ = "linked_list", "cursor"
        linked_list: 'LinkedList'
        cursor: LinkedNode

        def __next__(self):
            if self.linked_list.is_off(self.cursor):
                raise StopIteration()
            else:
                val = self.linked_list.get_value(self.cursor)
                self.cursor = self.linked_list.next_loc(self.cursor)
                return val

    def __iter__(self) -> Iterator[Any]:
        result = LinkedList._Iter()
        result.linked_list = self
